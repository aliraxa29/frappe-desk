    ...
    <template #header>
      <div class="flex items-center justify-between gap-4 w-full py-4">
        <div class="flex items-center gap-3">
          ...
          <button v-if="selectedModule" @click="selectedModule = null"
          ...
          <h2 class="text-lg font-semibold text-slate-800 dark:text-white">
            {{ selectedModule ? formatLabel(selectedModule.label || selectedModule.name) : formatLabel(moduleName) }}
          </h2>
        </div>
        <!-- Refresh button -->
        <button 
        ...
        >
          <svg class="w-5 h-5 text-slate-600 dark:text-slate-400" :class="{ 'animate-spin': workspaceLoading }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <template #content>
      ...
      <div v-if="loading" class="flex items-center justify-center h-[calc(100vh-200px)]">
        <div class="flex flex-col items-center gap-4">
          ...
          <div class="text-sm text-slate-500 dark:text-slate-400">Loading modules...</div>
        </div>
      </div>

      <!-- Module Icons Grid (Odoo-style centered view) -->
      <div v-else-if="!selectedModule" class="flex items-center justify-center min-h-[calc(100vh-200px)] p-8">
        <div class="w-full max-w-5xl">
          ...
          <div v-if="modules.length === 0"
            ...
            <h3 class="text-xl font-semibold text-slate-700 dark:text-slate-100">No Modules Found</h3>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-2 max-w-md">
              This app has no modules available. Try installing some modules or check your permissions.
            </p>
          </div>

          <!-- Modules Grid -->
          <div v-else class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6 justify-items-center">
            <div 
              v-for="module in modules" 
              :key="module.name" 
              @click="() => selectModule(module, false)" 
              class="group flex flex-col items-center gap-4 p-6 w-full max-w-40 rounded-2xl cursor-pointer
                     bg-white dark:bg-slate-900 border-2 border-transparent
                     hover:border-violet-400 hover:shadow-xl hover:shadow-violet-500/10
                     transition-all duration-300 hover:-translate-y-1">
              <!-- Module Icon -->
              <div class="relative">
                <div class="w-20 h-20 rounded-2xl bg-linear-to-br from-violet-500 to-purple-600
                            flex items-center justify-center shadow-lg shadow-violet-500/30
                            group-hover:scale-110 group-hover:shadow-xl group-hover:shadow-violet-500/40
              ...
              <!-- Module Label -->
              <span class="text-sm font-semibold text-slate-700 dark:text-slate-200 text-center
      ...
      <div v-else class="p-6 space-y-10">
        ...
        <template v-else>
          ...
          <div v-if="workspaceContent.shortcuts && workspaceContent.shortcuts.length > 0" class="space-y-4">
            <div class="flex items-center gap-3">
              <div class="w-8 h-8 rounded-lg bg-linear-to-br from-violet-500 to-purple-600 flex items-center justify-center">
                <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
                </svg>
              </div>
              <h3 class="text-base font-semibold text-slate-800 dark:text-white">Quick Actions</h3>
            </div>
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
              <div
                ...
                class="group relative overflow-hidden rounded-xl cursor-pointer
                       bg-linear-to-br from-white to-slate-50 dark:from-slate-800 dark:to-slate-900
                       border border-slate-200 dark:border-slate-700
                       hover:shadow-lg hover:shadow-violet-500/10 hover:border-violet-400
              ...
              >
                <div class="p-4 flex flex-col items-center gap-3">
                  <div
                    ...
                    :style="{ 
                      background: `linear-gradient(135deg, ${shortcut.color || '#8B5CF6'}15, ${shortcut.color || '#8B5CF6'}30)`,
                      border: `1px solid ${shortcut.color || '#8B5CF6'}40`
                    }"
                  >
                    <span class="text-2xl">{{ shortcut.icon || '⚡' }}</span>
                  </div>
                  <span class="text-sm font-medium text-slate-700 dark:text-slate-200 text-center leading-tight
                               group-hover:text-violet-600 dark:group-hover:text-violet-400 transition-colors">
                    {{ shortcut.label || shortcut.name }}
                  </span>
                </div>
                <!-- Subtle gradient overlay on hover -->
                <div class="absolute inset-0 bg-linear-to-br from-violet-500/0 to-purple-500/0 
                            group-hover:from-violet-500/5 group-hover:to-purple-500/5 transition-all duration-300"></div>
              </div>
            </div>
          </div>

          <!-- Number Cards Section - KPI Stats -->
          <div v-if="workspaceContent.number_cards && workspaceContent.number_cards.length > 0" class="space-y-4">
            <div class="flex items-center gap-3">
              <div class="w-8 h-8 rounded-lg bg-linear-to-br from-emerald-500 to-teal-600 flex items-center justify-center">
                <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
                </svg>
              </div>
              <h3 class="text-base font-semibold text-slate-800 dark:text-white">Key Metrics</h3>
            </div>
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
              <div
                v-for="(card, index) in workspaceContent.number_cards"
                :key="card.name"
                class="relative overflow-hidden rounded-xl p-5
              ...
              >
                <div class="flex flex-col">
                  ...
                  <span class="text-2xl font-bold text-slate-800 dark:text-white mt-1">--</span>
                </div>
                <!-- Decorative gradient corner -->
                <div
                  class="absolute -top-8 -right-8 w-16 h-16 rounded-full opacity-20"
                  :style="{ background: `linear-gradient(135deg, ${getNumberCardColor(index)})` }"
                ></div>
              </div>
            </div>
          </div>
          ...
          <div v-if="workspaceContent.cards && workspaceContent.cards.length > 0" class="space-y-6">
            <div class="flex items-center gap-3">
              ...
              <h3 class="text-base font-semibold text-slate-800 dark:text-white">Modules</h3>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <div
                v-for="card in workspaceContent.cards"
                :key="card.label"
                class="rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700
              ...
              >
                ...
                <div class="px-5 py-4 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
                  <div class="flex items-center gap-3">
                    ...
                    <h4 class="font-semibold text-slate-800 dark:text-white">{{ card.label }}</h4>
                  </div>
                </div>
                <!-- Card Links -->
                <div class="p-3">
                  <div class="space-y-1">
                    ...
                    >
                      ...
                      <span class="text-sm text-slate-700 dark:text-slate-300 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                        {{ link.label || link.name }}
                      </span>
                      <svg class="w-4 h-4 text-slate-400 ml-auto opacity-0 group-hover:opacity-100 transition-opacity" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <!-- Charts Section -->
          <div v-if="workspaceContent.charts && workspaceContent.charts.length > 0" class="space-y-4">
            <div class="flex items-center gap-3">
              ...
              <h3 class="text-base font-semibold text-slate-800 dark:text-white">Charts & Analytics</h3>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div
                v-for="chart in workspaceContent.charts"
                :key="chart.name"
                class="rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-6
                       hover:shadow-md transition-all duration-300"
              >
                <div class="flex items-center justify-between mb-4">
                  <h4 class="font-medium text-slate-800 dark:text-white">{{ chart.label || chart.name }}</h4>
                  <span class="text-xs text-slate-500 bg-slate-100 dark:bg-slate-700 px-2 py-1 rounded">{{ chart.chart_name }}</span>
                </div>
                <!-- Chart Placeholder -->
                <div class="h-40 bg-slate-50 dark:bg-slate-900 rounded-lg flex items-center justify-center border border-dashed border-slate-200 dark:border-slate-700">
                  <span class="text-sm text-slate-400">Chart: {{ chart.chart_name }}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <!-- Quick Lists Section -->
          <div v-if="workspaceContent.quick_lists && workspaceContent.quick_lists.length > 0" class="space-y-4">
            <div class="flex items-center gap-3">
              <div class="w-8 h-8 rounded-lg bg-linear-to-br from-pink-500 to-rose-600 flex items-center justify-center">
                <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 10h16M4 14h16M4 18h16" />
                </svg>
              </div>
              <h3 class="text-base font-semibold text-slate-800 dark:text-white">Quick Lists</h3>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div
                ...
                class="group rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-4
                       hover:border-pink-400 hover:shadow-md transition-all duration-300 cursor-pointer"
              >
                <div class="flex items-center gap-3">
                  ...
                  <div class="flex-1 min-w-0">
                    <h4 class="font-medium text-slate-800 dark:text-white truncate group-hover:text-pink-600 dark:group-hover:text-pink-400 transition-colors">
                      {{ list.label || list.name }}
                    </h4>
                    <p class="text-xs text-slate-500 dark:text-slate-400 truncate">{{ list.document_type }}</p>
                  </div>
                  <svg class="w-5 h-5 text-slate-400 group-hover:text-pink-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                  </svg>
                </div>
              </div>
          ...
          <div v-if="filteredDoctypes.length > 0" class="space-y-4">
            <div class="flex items-center gap-3">
              <div class="w-8 h-8 rounded-lg bg-linear-to-br from-cyan-500 to-blue-600 flex items-center justify-center">
                <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
              </div>
              <h3 class="text-base font-semibold text-slate-800 dark:text-white">Document Types</h3>
            </div>
            <div class="space-y-2">
              <div
                ...
                class="group flex items-center gap-4 p-4 rounded-lg cursor-pointer
                       bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700
                       hover:border-cyan-400 hover:bg-cyan-50 dark:hover:bg-slate-700/50 hover:shadow-sm
              ...
              >
                ...
                <svg class="w-4 h-4 text-slate-400 group-hover:text-cyan-500 transition-colors flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                </svg>
              </div>
            </div>
          ...
          <div v-if="filteredReports.length > 0" class="space-y-4">
            <div class="flex items-center gap-3">
              <div class="w-8 h-8 rounded-lg bg-linear-to-br from-purple-500 to-indigo-600 flex items-center justify-center">
                <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
              </div>
              <h3 class="text-base font-semibold text-slate-800 dark:text-white">Reports</h3>
            </div>
            <div class="space-y-2">
              <router-link
                ...
                class="group flex items-center gap-4 p-4 rounded-lg
                       bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700
                       hover:border-purple-400 hover:bg-purple-50 dark:hover:bg-slate-700/50 hover:shadow-sm
                       transition-all duration-200"
              >
                ...
                <svg class="w-4 h-4 text-slate-400 group-hover:text-purple-500 transition-colors flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                </svg>
              </router-link>
            </div>
          ...
          <div v-if="filteredPages.length > 0" class="space-y-4">
            <div class="flex items-center gap-3">
              ...
              <h3 class="text-base font-semibold text-slate-800 dark:text-white">Pages</h3>
            </div>
            <div class="space-y-2">
              <router-link
                ...
                class="group flex items-center gap-4 p-4 rounded-lg
                       bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700
                       hover:border-green-400 hover:bg-green-50 dark:hover:bg-slate-700/50 hover:shadow-sm
                       transition-all duration-200"
              >
          ...
          <div v-if="filteredDashboards.length > 0" class="space-y-4">
            <div class="flex items-center gap-3">
              <div class="w-8 h-8 rounded-lg bg-linear-to-br from-orange-500 to-red-600 flex items-center justify-center">
                <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" />
                </svg>
              </div>
              <h3 class="text-base font-semibold text-slate-800 dark:text-white">Dashboards</h3>
            </div>
            <div class="space-y-2">
              <router-link
                ...
                class="group flex items-center gap-4 p-4 rounded-lg
                       bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700
                       hover:border-orange-400 hover:bg-orange-50 dark:hover:bg-slate-700/50 hover:shadow-sm
                       transition-all duration-200"
              >
                ...
                <svg class="w-4 h-4 text-slate-400 group-hover:text-orange-500 transition-colors flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                </svg>
              </router-link>
            </div>
          ...
          <div v-if="isWorkspaceEmpty" class="flex flex-col items-center justify-center h-64 text-center">
            ...
            <h3 class="text-lg font-semibold text-slate-700 dark:text-slate-100">Empty Workspace</h3>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-2 max-w-md">
              This module has no items configured yet. Add shortcuts, doctypes, or reports to get started.
            </p>
          </div>
        </template>
      </div>
...
const isWorkspaceEmpty = computed(() => {
  return (
    ...
    (!workspaceContent.value.number_cards || workspaceContent.value.number_cards.length === 0) &&
...
function navigateToQuickList(list: { name: string; label: string; document_type: string; quick_list_filter?: string }) {
  router.push({
    name: 'ListView',
