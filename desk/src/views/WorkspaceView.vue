<template>
    <div class="flex flex-col min-h-full bg-gray-50 dark:bg-gray-900 ml-64">
        <!-- Header -->
        <header class="bg-white dark:bg-gray-950 border-b border-slate-200 dark:border-slate-800 px-6 py-4">
            <div class="flex items-center gap-3">
                <span v-if="workspaceContent.icon" class="text-2xl">{{ getIcon(workspaceContent.icon) }}</span>
                <div>
                    <h1 class="text-xl font-semibold text-slate-900 dark:text-white">
                        {{ workspaceContent.label || workspaceContent.name || route.params.workspace }}
                    </h1>
                </div>
            </div>
        </header>
        ...
        <div v-else class="flex-1 p-6 space-y-8">
            ...
            <section v-if="workspaceContent.shortcuts.length > 0">
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
                    ...
                    >
                        <div class="flex items-center gap-3">
                            <span class="text-xl" :class="shortcut.color ? `text-${shortcut.color}-500` : 'text-slate-600 dark:text-slate-400'">
                        ...
                        <p v-if="shortcut.type" class="mt-2 text-xs text-slate-500 dark:text-slate-400">
                            {{ shortcut.type }}
                        </p>
                    </div>
                </div>
            ...
            <section v-if="workspaceContent.cards.length > 0">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    >
                        <!-- Card Header -->
                        <div class="px-4 py-3 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-gray-850">
                            <div class="flex items-center gap-2">
                                <span v-if="card.icon" class="text-lg">{{ getIcon(card.icon) }}</span>
                                <h3 class="font-semibold text-slate-800 dark:text-slate-200">{{ card.label }}</h3>
                            </div>
                        </div>
                        ...
                        <div class="divide-y divide-slate-100 dark:divide-slate-700">
                            ...
                            >
                                <span class="text-base text-slate-500">{{ getIcon(link.icon) || '📄' }}</span>
                                <div class="flex-1 min-w-0">
                                    <span class="text-sm font-medium text-slate-700 dark:text-slate-300 truncate block">
                                        {{ link.label }}
                                    </span>
                                    <span v-if="link.description" class="text-xs text-slate-500 dark:text-slate-400 truncate block">
                                        {{ link.description }}
                                    </span>
                                </div>
            ...
            <section v-if="workspaceContent.charts.length > 0">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    >
                        <h3 class="font-medium text-slate-800 dark:text-slate-200 mb-3">{{ chart.label }}</h3>
                        <div class="h-40 bg-slate-100 dark:bg-gray-700 rounded-lg flex items-center justify-center text-slate-400">
            ...
            <section v-if="workspaceContent.number_cards.length > 0">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div 
                        ...
                        class="bg-white dark:bg-gray-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 shadow-sm"
                    >
                        <div class="text-sm text-slate-500 dark:text-slate-400">{{ nc.label }}</div>
                        <div class="text-2xl font-bold text-slate-800 dark:text-white mt-1">--</div>
                    </div>
                </div>
            </section>
            ...
            <section v-if="workspaceContent.quick_lists.length > 0">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    >
                        <h3 class="font-medium text-slate-800 dark:text-slate-200">{{ ql.label }}</h3>
...
async function loadWorkspace() {
    ...
    loading.value = true
    try {
        workspaceContent.value = await desktopAPI.getWorkspaceContent(decodeURIComponent(workspaceName))
    } catch (error) {
...
async function handleShortcutClick(shortcut: WorkspaceShortcut) {
    ...
    if (type === 'doctype' && linkTo) {
        ...
        if (isSingle) {
            router.push({ name: 'EditForm', params: { app: appName, doctype: linkTo, name: linkTo } })
        } else {
