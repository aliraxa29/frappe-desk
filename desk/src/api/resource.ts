...
class FrappeClient {
  ...
  // Create document
  async createDocument(doctype: string, data: Record<string, any>): Promise<Document> {
    try {
  ...
  // Update document
  async updateDocument(doctype: string, name: string, data: Record<string, any>): Promise<Document> {
    try {
      const response = await desk.call({
  ...
  async hasPermission(doctype: string, action: string = 'read'): Promise<boolean> {
    try {
      const response = await desk.call({
