<template>
	<nav
		class="sticky top-0 z-40 flex items-center justify-between gap-6 px-6 py-1 bg-slate-900/90 dark:bg-gray-950 backdrop-blur border-b border-slate-800 dark:border-slate-700 text-white transition-colors"
	>
		<div class="flex items-center min-w-0 gap-3">
			<button
				@click="goHome"
				class="text-white hover:text-white shrink-0 w-20 flex items-center gap-2 py-2 rounded-lg hover:bg-slate-800 dark:hover:bg-slate-700 cursor-pointer transition"
			>
		...
		<div class="hidden md:flex flex-1 max-w-sm mx-auto">
			<div class="relative w-full cursor-pointer" @click="openCommandDialog">
				...
				/>
				<span
					class="absolute right-3 top-1/2 -translate-y-1/2 px-2 py-1 text-xs font-mono text-slate-400 bg-slate-700 dark:bg-slate-600 border border-slate-600 dark:border-slate-500 rounded dark:text-white"
				>
					⌘K
				</span>
			</div>
		...
		<div class="relative user-profile-menu">
			...
			>
				<div
					class="w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold text-white shrink-0 bg-linear-to-br from-blue-500 to-purple-500"
				>
					{{ userInitials }}
				</div>
			...
			<transition name="fade">
				<div
					v-if="showUserMenu"
					class="user-menu absolute right-0 mt-2 w-64 z-50 bg-slate-800 dark:bg-slate-800 border border-slate-700 dark:border-slate-600 rounded-lg shadow-xl p-2 text-slate-100 dark:text-slate-100"
				>
					<div class="flex items-center gap-3 px-4 py-3">
						<div
							class="w-12 h-12 rounded-full flex items-center justify-center font-semibold text-white shrink-0 bg-linear-to-br from-blue-500 to-purple-500"
						>
							{{ userInitials }}
						</div>
					...
					<button
						@click="handleLogout"
						class="w-full flex items-center gap-3 px-4 py-2 rounded-md text-slate-300 hover:text-white hover:bg-slate-700 dark:hover:bg-slate-600 transition cursor-pointer"
					>
						<svg
