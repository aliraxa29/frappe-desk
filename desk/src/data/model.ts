...
export interface Model {
    ...
    with_doctype(doctype: string, callback?: (result: any) => any, async?: boolean): Promise<void>;
    ...
    trigger(fieldname: string, value: any, doc: any, skip_dirty_trigger?: boolean): Promise<void>;
...
export const model: Model = {
        ...
        desk.realtime.on("doc_update", function (data) {
            ...
            if (doc) {
                ...
                if (
                    ...
                    if (data.modified !== cur_frm.doc.modified && !desk.ui.form.is_saving) {
                        if (!cur_frm.is_dirty()) {
    ...
    is_non_std_field: function (fieldname) {
        return ![...desk.model.std_fields_list, ...desk.model.child_table_field_list].includes(
            fieldname
        );
    },
    ...
    with_doctype: function (doctype: string, callback = undefined, async = false) {
    ...
    get_docinfo: function (doctype, name) {
        return (desk.model.docinfo[doctype] && desk.model.docinfo[doctype][name]) || null;
    },
    ...
    has_workflow: function (doctype) {
        return desk.get_list("Workflow", { document_type: doctype, is_active: 1 }).length;
    },
    ...
    set_value: function (
        ...
        $.each(to_update, (key, value) => {
            if (doc && doc[key] !== value) {
                ...
                doc[key] = value;
                tasks.push(() => desk.model.trigger(key, value, doc, skip_dirty_trigger));
            } else {
    ...
    validate_missing: function (doc, fieldname) {
        if (!doc[fieldname]) {
            desk.throw(
                __("Please specify") +
                ": " +
                __(desk.meta.get_label(doc.doctype, fieldname, doc.parent || doc.name))
            );
        }
    },
