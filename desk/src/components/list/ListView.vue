<template>
	<div class="w-full bg-white dark:bg-gray-950 rounded-lg">
		...
		<div
			v-else-if="error"
			class="p-4 rounded-lg border border-red-200 bg-red-50 dark:bg-red-900/20 dark:border-red-700 text-sm text-red-700 dark:text-red-300"
		>
			{{ error }}
		</div>
		...
		<template v-else>
			<!-- Filters Bar -->
			<div
				class="flex flex-wrap items-center gap-3 p-4 border-b border-slate-200 dark:border-slate-800"
			>
				<!-- Quick Filters (Left) -->
			...
			<div class="overflow-x-auto">
				<table class="w-full">
					<thead
						class="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-800"
					>
						<tr>
							<!-- Checkbox column -->
						...
						>
							...
							>
								...
								<template v-if="col.type === 'Subject'">
									<div
										class="font-medium text-slate-900 hover:text-blue-600 dark:text-white dark:hover:text-gray-400"
									>
										{{ getSubjectValue(row) }}
									</div>
									<div
										v-if="meta?.title_field && meta.title_field !== 'name'"
...
const hasActiveFilters = computed(() => {
	const quick = Object.values(activeFilters.value).some(
...
function getQueryField(
	row: { id: string; fieldname: string },
	part?: "from" | "to",
): Field | null {
...
function getCellClass(col: ListColumn): string {
	if (
		col.df?.fieldtype === "Currency" ||
		col.df?.fieldtype === "Float" ||
		col.df?.fieldtype === "Int"
	) {
