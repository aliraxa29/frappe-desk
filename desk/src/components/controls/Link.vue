	...
	<div class="flex flex-col relative">
		...
		<div class="relative">
			...
			>
				...
				/>
				<button
					v-if="modelValue && !field.read_only"
				...
				<button
					v-if="modelValue && !field.read_only"
					class="px-2 py-2 transition-colors cursor-pointer"
					title="Clear"
					@click="clearValue"
				>
			...
			resultItems.value = data
				.map((item: any): LinkItem | null => {
					...
					if (typeof item === "object" && item !== null) {
						if (item.value?.includes("__link_option")) return null;
						return {
				...
				.filter((item): item is LinkItem => item !== null && item.value !== undefined);
		} else {
...
function createNewDoc() {
	const meta = locals.getMeta(props.field.options!);
	if (meta && meta.quick_entry) {
	}
}
