/**
* Dashboard Chart Type Configurator
* Helps users create and configure different chart types
*/
    ...
    <div class="space-y-6">
        ...
        <div class="space-y-3">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3">
                <button v-for="type in chartTypes" :key="type.value" @click="selectedType = type.value" :class="[
        ...
        <div v-if="selectedType" class="space-y-3">
            ...
            <div
                ...
                <apexchart :type="selectedType as any" :options="previewOptions" :series="previewData" height="400" />
            </div>
        </div>
        ...
        <div class="space-y-4">
            ...
            <!-- Title -->
            <div class="space-y-2">
                <label class="text-sm font-medium text-slate-700 dark:text-slate-300">Chart Title</label>
                <input v-model="config.title" type="text" placeholder="Enter chart title"
            ...
            <!-- Data Source -->
            <div class="space-y-2">
                <label class="text-sm font-medium text-slate-700 dark:text-slate-300">Data Source (DocType)</label>
                <input v-model="config.source" type="text" placeholder="e.g., Invoice, Sales Order"
                    class="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>

            <!-- Show Values -->
            <div class="flex items-center gap-3">
                <input v-model="config.showValues" type="checkbox" id="showValues"
                    class="w-4 h-4 rounded border-slate-300 dark:border-slate-600 text-blue-600" />
                <label for="showValues" class="text-sm font-medium text-slate-700 dark:text-slate-300">
            ...
            <div v-if="isTimeSeriesChart" class="flex items-center gap-3">
                <input v-model="config.timeseries" type="checkbox" id="timeseries"
                    class="w-4 h-4 rounded border-slate-300 dark:border-slate-600 text-blue-600" />
                <label for="timeseries" class="text-sm font-medium text-slate-700 dark:text-slate-300">
                    Time series data
                </label>
            </div>
            ...
            <!-- Colors -->
            <div class="space-y-2">
                <label class="text-sm font-medium text-slate-700 dark:text-slate-300">Colors (comma separated
                    hex)</label>
                <input v-model="config.colors" type="text" placeholder="#3b82f6,#ef4444,#10b981"
                    class="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>

            <!-- Height -->
            <div class="space-y-2">
                <label class="text-sm font-medium text-slate-700 dark:text-slate-300">Chart Height (px)</label>
                <input v-model.number="config.height" type="number" min="200" max="1000"
                    class="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
        </div>
        ...
        <!-- Generated JSON -->
        <div class="space-y-3">
            <label class="text-sm font-semibold text-slate-900 dark:text-white">Generated Configuration</label>
            <div class="bg-slate-900 dark:bg-slate-950 rounded-lg p-4">
                <pre class="text-xs text-slate-100 overflow-x-auto">{{ JSON.stringify(generatedConfig, null, 2) }}</pre>
            </div>
...
const previewData = computed(() => {
    ...
    if (['scatter', 'bubble'].includes(selectedType.value)) {
        return [
            {
                name: 'Bubble',
                data: [[10, 40], [40, 60], [30, 50], [70, 80], [50, 70]]
            }
        ]
    }
