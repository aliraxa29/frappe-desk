		...
		<transition-group name="chart-fade" tag="div" class="contents">
			...
			>
				<DashboardChart
					...
					@error="handleChartError(chartName, $event)"
					@loaded="handleChartLoaded(chartName, $event)"
				/>
			</div>
		</transition-group>
	</div>
...
const gridClass = computed(() => {
	...
	return [gridColsClass[props.columns] || gridColsClass.responsive, gapClass[props.gap]];
});
