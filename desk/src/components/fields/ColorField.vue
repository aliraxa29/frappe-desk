<template>
  <div class="mb-4 flex flex-col relative">
    ...
    <div class="relative">
      ...
      <div class="absolute left-2 top-1/2 -translate-y-1/2 w-4 h-4 rounded cursor-pointer border border-gray-300"
        :class="{ 'bg-[url(data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAwAAAAMCAIAAADZF8uwAAAAGUlEQVQYV2M4gwH+YwCGIasIUwhT25BVBADtzYNYrHvv4gAAAABJRU5ErkJggg==)] bg-contain': !modelValue }"
        :style="{ backgroundColor: modelValue || 'transparent' }" @click="showPicker = !showPicker" />
    </div>
    ...
    <Transition name="picker">
      <div v-if="showPicker" ref="pickerRef"
        class="absolute top-full left-0 mt-2 w-52.5 bg-white border border-gray-300 rounded-lg shadow-lg p-3 z-50">
        <!-- Swatches Section -->
        <div>
          <div class="text-xs text-gray-500 font-medium mb-2 uppercase tracking-wide">Swatches</div>
          <div class="flex flex-wrap gap-2 mt-2 mb-2">
            <div v-for="(swatch, idx) in swatches" :key="idx"
              class="h-5 w-5 rounded-full cursor-pointer hover:scale-110 transition-transform"
              :style="{ backgroundColor: swatch }" tabindex="0" @click="selectSwatch(swatch)"
              @keydown.enter.space.prevent="selectSwatch(swatch)" />
          </div>
        </div>

        <!-- Color Picker Section -->
        <div class="text-xs text-gray-500 font-medium mb-2 mt-2 uppercase tracking-wide">Color Picker</div>

        <!-- 2D Saturation/Brightness Picker -->
        <div ref="colorMapRef" class="relative w-full h-35 rounded-md mb-2 cursor-crosshair color-map-gradient"
          :style="{ background: colorMapBackground, color: modelValue || 'transparent' }"
          @mousedown="startSaturationPick" @touchstart="startSaturationPick" @click="clickSaturationPick">
          <div class="color-selector absolute pointer-events-none" :style="colorSelectorStyle" />
        </div>

        <div ref="hueSliderRef"
          class="relative w-full h-3.5 rounded-full cursor-pointer border border-gray-200 hue-gradient"
          :style="{ color: `hsl(${hue}, 100%, 50%)` }" @mousedown="startHuePick" @touchstart="startHuePick"
          @click="clickHuePick">
          <div class="hue-selector absolute pointer-events-none" :style="hueSelectorStyle" />
        </div>
      </div>
    </Transition>
  </div>
...
watch(() => modelValue.value, (newColor) => {
...
onMounted(() => {
  ...
  const handleClickOutside = (event: Event) => {
    if (showPicker.value && pickerRef.value && !pickerRef.value.contains(event.target as Node) && !inputRef.value?.contains(event.target as Node)) {
