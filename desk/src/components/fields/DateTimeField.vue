        ...
        <div class="mb-3">
          ...
          <div class="grid grid-cols-7 gap-1">
            <button v-for="d in calendarDays" :key="d" :disabled="d === 0" :class="getDayButtonClass(d)"
              @click="selectDay(d)"
              class="text-xs py-1 cursor-pointer rounded transition-colors disabled:cursor-default hover:bg-gray-300">
              {{ d || '' }}
            </button>
          </div>
        </div>
...
function getDayButtonClass(d: number): string {
  ...
  if (isCurrentDay) return 'border border-blue-400 text-slate-900 font-medium'
