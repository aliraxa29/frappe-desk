<template>
  <div class="mb-4">
    <label :for="id" class="flex items-start gap-3 cursor-pointer select-none transition-opacity"
      ...
      <div class="relative mt-1">
        ...
        />
        <!-- Switch background -->
        <div class="h-5 w-9 rounded-full transition-all bg-slate-300 dark:bg-slate-600 peer-checked:bg-blue-600 dark:peer-checked:bg-blue-600 peer-focus:ring-2 peer-focus:ring-blue-600/50 dark:peer-focus:ring-blue-500/50">
        </div>
        <!-- Switch toggle -->
        <div class="absolute left-0.5 top-0.5 h-4 w-4 rounded-full bg-white dark:bg-slate-100 shadow-sm transition-transform peer-checked:translate-x-4 peer-disabled:opacity-50"></div>
      </div>
      <div class="flex flex-col">
        ...
        <span v-if="field.description" class="text-[0.85rem] text-slate-600 dark:text-slate-400 mt-0.5">
          {{ field.description }}
        </span>
      </div>
    </label>
