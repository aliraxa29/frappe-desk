<template>
	<div class="modal-field-editor">
		...
		<template v-else-if="['Float', 'Currency', 'Percent'].includes(field.fieldtype)">
			<div class="number-input-wrapper">
				...
				<input
					...
					:value="value"
					@input="
						emit('update', parseFloat(($event.target as HTMLInputElement).value) || 0)
					"
					:step="field.precision ? Math.pow(10, -field.precision) : 0.01"
					class="form-input number-input"
					:class="{
						'has-prefix': field.fieldtype === 'Currency',
						'has-suffix': field.fieldtype === 'Percent',
					}"
					:required="field.reqd"
				/>
		...
		<template
			v-else-if="
				['Small Text', 'Long Text', 'Text Editor', 'Text', 'Code', 'JSON'].includes(
					field.fieldtype,
				)
			"
		>
