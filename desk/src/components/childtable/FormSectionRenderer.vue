<template>
  <div class="mb-6 last:mb-0" :class="{ 'border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden': section.collapsible }">
    <!-- Collapsible section -->
    <div v-if="section.collapsible" class="bg-white dark:bg-slate-900">
      ...
      >
        ...
        <span class="text-[0.9375rem] font-semibold text-slate-800 dark:text-slate-200">{{ section.label || 'Section' }}</span>
      </button>
      
      <div v-if="isOpen" class="p-4">
        <p v-if="section.description" class="text-[0.8125rem] text-slate-500 dark:text-slate-400 mb-4">{{ section.description }}</p>
    ...
    <template v-else>
      <div v-if="section.label" class="mb-4 pb-3 border-b border-slate-200 dark:border-slate-700">
        <h3 class="text-[0.9375rem] font-semibold text-slate-800 dark:text-slate-200 m-0">{{ section.label }}</h3>
        <p v-if="section.description" class="text-[0.8125rem] text-slate-500 dark:text-slate-400 mt-1 mb-0">{{ section.description }}</p>
      </div>
...
watch(() => props.section.collapsed, (newVal) => {
  isOpen.value = !newVal
})
