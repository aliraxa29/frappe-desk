<template>
    <div @click="handleSelect" :class="containerClass">
        ...
        <div class="relative h-28 bg-linear-to-br from-slate-200 to-slate-300 dark:from-slate-800 dark:to-slate-950">
            ...
            <!-- Floating Icon -->
            <div class="absolute -bottom-5 left-4 flex h-11 w-11 items-center justify-center rounded-xl
        ...
        <div class="flex h-[calc(100%-7rem)] flex-col px-5 pb-5 pt-8">
            <h3 class="truncate text-base font-semibold text-slate-900 dark:text-slate-100" :title="app.title">
                {{ app.title }}
            </h3>

            <p class="mt-1 line-clamp-3 text-sm text-slate-600 dark:text-slate-400" :title="app.description || __('No description available')">
...
const containerClass = computed(() => [
    ...
    props.selectable ? 'cursor-pointer hover:-translate-y-1 hover:scale-[1.02] hover:border-violet-500 hover:shadow-2xl hover:shadow-violet-500/20' : 'cursor-default'
])
