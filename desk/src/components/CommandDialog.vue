      ...
      <div v-if="isOpen" class="command-overlay" @click="close">
        <div class="command-modal" @click.stop>
          ...
          <div class="command-header">
            <svg class="command-search-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="command-results" ref="resultsRef">
            ...
            <!-- No results -->
            <div v-else-if="!loading && searchQuery && totalResults === 0" class="command-no-results">
            ...
            <div v-else-if="totalResults > 0" class="command-results-container">
              <!-- Recent Group -->
              <div v-if="groupedResults.recent.length > 0 && !searchQuery" class="command-group">
                ...
                <button 
                  ...
                  :class="['command-item', { 'command-item-active': selectedIndex === getItemIndex('recent', index) }]"
                  @click="selectItem(item)" 
                  @mouseenter="selectedIndex = getItemIndex('recent', index)"
                  :ref="el => setItemRef(el, getItemIndex('recent', index))"
                >
                  <div class="command-item-icon">{{ getTypeIcon(item.type) }}</div>
                  <div class="command-item-content">
                    <div class="command-item-label">{{ item.label }}</div>
                    <div v-if="item.description" class="command-item-desc">{{ item.description }}</div>
                  </div>
                </button>
              </div>
              ...
              <div v-if="groupedResults.new.length > 0" class="command-group">
                ...
                <button 
                  ...
                  :key="`new-${item.name}`"
                  :class="['command-item', { 'command-item-active': selectedIndex === getItemIndex('new', index) }]"
                  @click="selectItem(item)" 
                ...
                >
                  ...
                  <div class="command-item-content">
                    <div class="command-item-label" v-html="item.markedLabel || item.label"></div>
                    <div v-if="item.module" class="command-item-desc">{{ item.module }}</div>
                  </div>
                </button>
              </div>
              ...
              <div v-if="groupedResults.calculator.length > 0" class="command-group">
                ...
                <button 
                  ...
                  :key="`calc-${index}`"
                  :class="['command-item', { 'command-item-active': selectedIndex === getItemIndex('calculator', index) }]"
                  @click="selectItem(item)" 
                  @mouseenter="selectedIndex = getItemIndex('calculator', index)"
                  :ref="el => setItemRef(el, getItemIndex('calculator', index))"
                >
                  <div class="command-item-icon command-item-icon-calc">=</div>
                  <div class="command-item-content">
                    <div class="command-item-label" v-html="item.markedLabel || item.label"></div>
                    <div class="command-item-desc">{{ __('Click to copy result') }}</div>
                  </div>
                </button>
              </div>

              <!-- Search In Group -->
              <div v-if="groupedResults['search-in'].length > 0" class="command-group">
                <div class="command-group-label">
                  <span class="command-group-icon">🔎</span>
                  {{ __('Search In') }}
                </div>
                <button 
                  ...
                  :class="['command-item', { 'command-item-active': selectedIndex === getItemIndex('search-in', index) }]"
                  @click="selectItem(item)" 
              ...
              <div v-if="groupedResults.doctype.length > 0" class="command-group">
                ...
                <button 
                  ...
                  :class="['command-item', { 'command-item-active': selectedIndex === getItemIndex('doctype', index) }]"
                  @click="selectItem(item)" 
                ...
                >
                  ...
                  <div v-if="searchQuery" class="command-item-score">{{ Math.round(item.score) }}</div>
                </button>
              </div>
              ...
              <div v-if="groupedResults.report.length > 0" class="command-group">
                ...
                <button 
                  ...
                  :class="['command-item', { 'command-item-active': selectedIndex === getItemIndex('report', index) }]"
                  @click="selectItem(item)" 
              ...
              <div v-if="groupedResults.workspace.length > 0" class="command-group">
                ...
                <button 
                  ...
                  :key="`ws-${item.name}`"
                  :class="['command-item', { 'command-item-active': selectedIndex === getItemIndex('workspace', index) }]"
                  @click="selectItem(item)" 
              ...
              <div v-if="groupedResults.page.length > 0" class="command-group">
                ...
                <button 
                  ...
                  :key="`page-${item.name}`"
                  :class="['command-item', { 'command-item-active': selectedIndex === getItemIndex('page', index) }]"
                  @click="selectItem(item)" 
              ...
              <div v-if="groupedResults.dashboard.length > 0" class="command-group">
                ...
                <button 
                  ...
                  :key="`dash-${item.name}`"
                  :class="['command-item', { 'command-item-active': selectedIndex === getItemIndex('dashboard', index) }]"
                  @click="selectItem(item)" 
              ...
              <div v-if="groupedResults.global.length > 0" class="command-group">
                ...
                <button 
                  ...
                  :class="['command-item', { 'command-item-active': selectedIndex === getItemIndex('global', index) }]"
                  @click="selectItem(item)" 
              ...
              <div v-if="groupedResults.help.length > 0" class="command-group">
                ...
                <button 
                  ...
                  :key="`help-${index}`"
                  :class="['command-item', { 'command-item-active': selectedIndex === getItemIndex('help', index) }]"
                ...
                >
                  ...
                  <div class="command-item-content">
                    <div class="command-item-label">{{ item.label }}</div>
                    <div class="command-item-desc">{{ __('View search shortcuts and tips') }}</div>
                  </div>
                </button>
              </div>
            </div>
        ...
        <div v-if="showHelp" class="help-dialog" @click.stop>
          ...
          <div class="help-dialog-content">
            <table class="help-table">
              <tbody>
                <tr>
                  <td><strong>{{ __('Create a new record') }}</strong></td>
                  <td><code>new {{ __('[doctype name]') }}</code></td>
                </tr>
                <tr>
                  <td><strong>{{ __('List a document type') }}</strong></td>
                  <td><code>{{ __('[doctype name]') }}</code></td>
                </tr>
                <tr>
                  <td><strong>{{ __('Search in a document type') }}</strong></td>
                  <td><code>{{ __('[text] in [doctype]') }}</code></td>
                </tr>
                <tr>
                  <td><strong>{{ __('Open a report') }}</strong></td>
                  <td><code>{{ __('[report name]') }}</code></td>
                </tr>
                <tr>
                  <td><strong>{{ __('Open a workspace') }}</strong></td>
                  <td><code>{{ __('[workspace name]') }}</code></td>
                </tr>
                <tr>
                  <td><strong>{{ __('Calculate') }}</strong></td>
                  <td><code>(55 + 434) / 4</code> {{ __('or') }} <code>=Math.sin(Math.PI/2)</code></td>
                </tr>
              </tbody>
            </table>
          </div>
...
const groupOrder = ['recent', 'new', 'calculator', 'search-in', 'doctype', 'report', 'workspace', 'page', 'dashboard', 'global', 'help'] as const
