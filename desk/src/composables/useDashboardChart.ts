...
import type { DashboardChart, ChartData, ApexChartOptions } from '../types/dashboard'
...
export function useDashboardChart(chartName: string) {
    ...
    const transformDataForChart = (items: any[]): ChartData[] => {
        ...
        items.forEach(item => {
            const key = item[chart.value?.number_of_groups ? `group_${1}` : 'name'] || 'Series'
            if (!grouped[key]) {
                grouped[key] = []
            }
        ...
        try {
            ...
            responses.forEach((Response, index) => {
                if (!Response.exc && Response.message) {
                    charts.value.set(chartNames[index] as string, Response.message as DashboardChart)
                }
            })
        } catch (err: any) {
