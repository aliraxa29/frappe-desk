...
export interface RealtimeEventMap {
  ...
  doc_viewers: (data: {
    doctype: string;
    name: string;
    viewers: Array<{ user: string; full_name: string }>;
  }) => void;
  open_docs: (data: {
    docs: Array<{ doctype: string; name: string; user: string }>;
  }) => void;
  [key: string]: any;
}
...
export class RealTimeClient {
  ...
  init(bootData: any) {
    ...
    if (!this.loggedIn) {
      console.warn(
        "[Realtime] Guest user detected, skipping Socket.IO connection",
      );
      return;
    }
  ...
  private getSocketUrl(): string {
    ...
    if (socketioHost) {
      return socketioPort
        ? `${protocol}://${socketioHost}:${socketioPort}`
        : `${protocol}://${socketioHost}`;
    }
  ...
  emit(event: string, ...args: any[]) {
    if (this.socket?.connected) {
      ...
      console.warn(
        `[Realtime] Attempted to emit '${event}' while disconnected`,
      );
    }
  }
