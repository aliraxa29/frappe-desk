...
export function __(txt: string, replace?: string | Record<string, any>, context: string | null = null): string {
    if (!txt) return txt;
    ...
    try {
        ...
        translated_text = translationStore.getTranslation(txt, context || undefined)
    } catch (e) {
 ...
 */
export function __n(singular: string, plural: string, count: number, replace?: Record<string, any>): string {
