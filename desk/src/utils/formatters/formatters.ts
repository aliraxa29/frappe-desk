...
export const formatters = {
  _right: function (value: any, options?: any) {
  ...
  _apply_custom_formatter: function (value: any, df: Field) {
    if (df) {
      const std_df =
        desk.meta.docfield_map[df.parent ?? ""] &&
        desk.meta.docfield_map[df.parent ?? ""][df.fieldname];
      if (
        std_df &&
        std_df.formatter &&
  ...
  Data: function (value: any, df: Field) {
    ...
    if (df && df.options == "IBAN") {
      if (!value) return "";
      return this.get_formatted_iban(value);
    }
  ...
  Currency: function (value, docfield, options, doc) {
    ...
    if (precision > 2) {
      ...
      if (decimals.length < 3 || decimals.length < precision) {
        const fraction =
          frappe.model.get_value(":Currency", currency, "fraction_units") ||
  ...
  Code: function (value) {
    return (
      "<pre>" + (value == null ? "" : $("<div>").text(value).html()) + "</pre>"
    );
  },
