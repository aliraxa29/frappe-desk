...
export const formatters: Record<string, FormatterFn> = {
  ...
  _apply_custom_formatter(value, df) {
    ...
    const std_df = window.desk.meta.docfield_map?.[df.parent]?.[df.fieldname];
  ...
  Float(value, df, options, doc) {
    ...
    let precision =
      df.precision ?? cint(desk.boot.sysdefaults?.float_precision) ?? null;
  ...
  Percent(value, df, options) {
    ...
    const precision =
      df.precision ?? cint(desk.boot.sysdefaults?.float_precision) ?? 2;
  ...
  Currency(value, df, options, doc) {
    ...
    return options?.only_value ? value : formatters._right(value, options);
  },
  ...
  Date(value) {
    ...
    return value ? desk.datetime.str_to_user(value, false, true) || "" : "";
  },
