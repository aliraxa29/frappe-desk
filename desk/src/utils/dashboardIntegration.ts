...
import { resource as desk } from "./resource";
...
export async function fetchDashboardCharts(
  ...
  const charts = await Promise.all(
    chartNames.map((name) => fetchDashboardChart(name)),
  );
 ...
 */
export async function saveDashboardChart(
  chart: Partial<DashboardChart> & { name: string },
): Promise<boolean> {
  try {
    await desk.call({
 ...
 */
export async function getDashboardChartsByModule(
  moduleName: string,
): Promise<DashboardChart[]> {
 ...
 */
export async function getDashboardChartsBySource(
  doctype: string,
): Promise<DashboardChart[]> {
  return listDashboardCharts({
    source: doctype,
  ...
  try {
    ...
    await desk.call({
      ...
      args: {
        ...
        name: dashboardName,
        fieldname: "charts",
        value: JSON.stringify([chartName, ...(chartLabel ? [chartLabel] : [])]),
      },
    });
...
export function exportChartDataToCSV(
  ...
  data.forEach((series) => {
    series.data.forEach((point) => {
      ...
      const yValue =
        typeof point.y === "number" ? point.y : JSON.stringify(point.y);
 ...
 */
export function getChartStatistics(data: ChartData[]): Record<string, any> {
  if (!data || data.length === 0) {
    return {};
  }
