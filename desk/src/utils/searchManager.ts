...
class SearchManager {
    ...
    try {
      ...
      this.validAppNames = new Set(
        this.installedApps.map(
          (app) =>
            app.module_name?.toLowerCase().replace(/\s+/g, "_") ||
      ...
      this.canCreate = this.doctypes
        .filter((dt) => !dt.issingle)
        .map((dt) => dt.name);
   ...
   */
  private fuzzySearchItems<
    T extends { name: string; label?: string; title?: string; module?: string },
  >(
    items: T[],
    query: string,
    ...
    items.forEach((item) => {
      ...
      const [matched, fuzzyScore, matches] = fuzzyMatch(
        query,
        searchText,
        true,
      );
      ...
      // 3. Contains query as a whole word (e.g., "Item" in "Sales Item")
        ...
        searchTextLower.includes(` ${queryLower}`) ||
        searchTextLower.includes(`${queryLower} `)
      ) {
        finalScore += 2000;
      }
      ...
      if (doctype?.issingle) {
        ...
        results.push({
          ...
          onclick: () => {
            router.push({
              name: "EditForm",
              params: { app, doctype: item.name, name: item.name },
            });
          },
        });
      } else {
      ...
      return {
        ...
        onclick: () => {
          if (report?.report_type === "Report Builder" && report.ref_doctype) {
            router.push(
              `/dashboard/${app}/${report.ref_doctype}/report/${item.name}`,
            );
          } else {
  ...
  getWorkspaces(query: string): SearchResult[] {
    ...
    return fuzzyResults.slice(0, 10).map(({ item, score, markedLabel }) => ({
      ...
      onclick: () => {
        const app =
          this.getValidAppName(item.module) ||
      ...
      return response.message.map((item: any) => {
        ...
        return {
          ...
          onclick: () => {
            router.push({
              name: "EditForm",
              params: { app, doctype: item.doctype, name: item.name },
            });
          },
        };
      });
