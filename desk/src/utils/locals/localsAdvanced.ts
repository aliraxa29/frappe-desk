 ...
 */
export function searchDocuments(doctype: string, searchTerm: string, searchFields: string[]): DeskDocument[] {
...
export function clearOldDocuments(doctype: string, daysOld: number = 30) {
	...
	let cleared = 0;
	for (const doc of docs) {
		if (doc.__last_sync_on && doc.__last_sync_on < cutoffDate && !doc.__unsaved) {
 ...
 */
export function mergeDocuments(doctype: string, targetName: string, sourceName: string) {
	const store = useLocalsStore();
...
): { valid: DeskDocument[]; invalid: { doc: DeskDocument; errors: string[] }[] } {
	const docs = getList(doctype);
...
export function getDocumentRelationships(doctype: string, name: string) {
	...
	const relationships = {
		parent: doc.parent && doc.parenttype ? { doctype: doc.parenttype, name: doc.parent } : null,
 ...
 */
export function detectConflicts(doctype: string): { conflicts: DeskDocument[]; count: number } {
	const docs = getList(doctype);
